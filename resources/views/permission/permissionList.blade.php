                    ...
                    <div class="portlet-body">
                                    ...
                                    @else
                                        @foreach ($list as $vo)
                                            <tr class="odd gradeX">
                                                ...
                                                {{--<td> {{$vo->updated_at}} </td>--}}
                                                <td>
                                                    <button type="button" class="btn btn-sm blue btn-outline" onclick="editPermission('{{$vo->id}}')">
                                                        编辑
                                                    </button>
                                                    <button type="button" class="btn btn-sm red btn-outline" onclick="deletePermission('{{$vo->id}}')">
                                                        删除
                                                    </button>
                                                </td>
                                            </tr>
                        ...
                        <div class="row">
                            <div class="col-md-4 col-sm-4">
                                <div class="dataTables_info" role="status" aria-live="polite">共 {{$list->total()}} 条行为</div>
                            </div>
    ...
    <script type="text/javascript">
        ...
        function deletePermission(id) {
            layer.confirm('确定删除权限行为？', {icon: 2, title:'警告'}, function(index) {
                $.post("/permission/deletePermission", {id:id, _token:'{{csrf_token()}}'}, function(ret) {
