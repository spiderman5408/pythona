                                    ...
                                    @else
                                        @foreach ($userList as $user)
                                            <tr class="odd gradeX {{$user->trafficWarning ? 'danger' : ''}}">
                                                <td> {{$user->id}} </td>
                                                <td> <a href="javascript:;" class="copySubscribeLink" data-clipboard-text="{{$user->sub_link}}" title="点击复制订阅链接">{{$user->username}}</a> </td>
                                                <td> <a href="javascript:;" class="copyShadowrocketLink" data-clipboard-text="{{$user->shadowrocket_link}}" title="点击复制Shadowrocket在线安装链接">{{$user->port ? $user->port : '未分配'}}</a> </td>
                                                <td> <a href="javascript:;" class="copyAllNodes" data-clipboard-text="{{$user->ssr_link}}" title="点击复制可用节点的SSR链接">{{$user->level_label}}</a> </td>
    ...
    <script type="text/javascript">
        ...
        function editUser(id) {
            @if(Request::getQueryString())
                window.location.href = '/admin/editUser/' + id + '?{!! Request::getQueryString() !!}';
            @else
        ...
        function doSearch(is_export) {
            ...
            window.location.href = '/admin/userList?id=' + id +'&username=' + username + '&wechat=' + wechat + '&qq=' + qq + '&address=' + address + '&phone=' + phone + '&port=' + port + '&expire_time=' + expire_time + '&level=' + level + '&pay_way=' + pay_way + '&status=' + status + '&enable=' + enable+ '&is_export=' + is_export;
        }
        ...
        function doExport(id) {
            window.location.href = '/admin/export/' + id;
        }
        ...
        function doMonitor(id) {
            window.location.href = '/admin/userMonitor/' + id;
        }
        ...
        function addTicket(username) {
            window.location.href = '/ticket/addTicket?username=' + username;
        }
        ...
        sub_clipboard.on('success', function(e) {
            layer.msg("成功复制该用户的订阅链接", {time: 1000});
        });
        ...
        // 复制订阅链接
        var shadowrocket_clipboard = new Clipboard('.copyShadowrocketLink');
        shadowrocket_clipboard.on('success', function(e) {
            layer.msg("成功复制该用户的Shadowrocket在线安装链接", {time: 1000});
        });
        shadowrocket_clipboard.on('error', function(e) {
            console.log(e);
        });
        ...
        copyAllNodes_clipboard.on('success', function(e) {
            layer.msg("复制成功，通过右键菜单倒入节点链接即可", {time: 1000});
        });
